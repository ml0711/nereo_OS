...
- **Realtime-Sync (Graph-Webhooks):** Änderungen in SharePoint halten den gecachten Index (KPIs/Datenraum-Liste) live. Webhook = NUR Trigger (`/api/graph/webhook`, clientState-abgesichert) → `clientState`-Check → dirty-Flag → 202 → entkoppelt: Delta-Query (`driveDelta`) → `isUnderRoot`-Filter → gezielter Re-Walk → Index-Merge (`sync.js`/`deltaThenRewalkAndSave`) → betroffene Datenräume als **veraltet markiert** (`last_change_at`); **KI-Analyse wird NICHT auto-neu gerechnet** (Token-Schutz), die UI fragt „neu analysieren?". **Self-Management (kein externer Scheduler):** die App hält die Subscription selbst am Leben — In-Process-Timer in `server.js`: Startup-Reconcile (8s nach Listen, self-bootstrap/-heal), Renewal-Check alle 6h, unabhängiger Delta-Poll alle 20min als Korrektheits-Backstop. Single-Flight über `graph_subscription.processing_at`-Lease (`claimSubscriptionForSync` vs `claimDirtySubscription`). Die CLIs `graph:subscribe`/`graph:sync` existieren weiter als manuelle Werkzeuge (Coolify Scheduled Tasks optional). Watchdog in `/healthz` (`sync.expiration`/`lastSyncAt`). Tabellen: `graph_subscription`, `dataroom_analysis.last_change_at`.
